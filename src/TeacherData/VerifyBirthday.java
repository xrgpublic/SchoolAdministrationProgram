...
public class VerifyBirthday {
    ...
    public boolean checkBirthdayDate(String birthday){
        //Student must be between the ages 20 and 80
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MM/dd/yyyy");
        LocalDate earliestDate =LocalDate.parse("05/01/1942", formatter);
        LocalDate latestDate = LocalDate.parse("05/01/2002", formatter);
        try {
            LocalDate birthdate = LocalDate.parse(birthday, formatter);
            System.out.println("Compareing birthday "+birthdate.compareTo(earliestDate)+ "  "+birthdate.compareTo(latestDate));
            //Return true if birthday is between 05/01/1942 and 05/01/2002
